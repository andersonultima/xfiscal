...
class Inicio extends Controller
{
    ...
    public function emissor()
    {
        ...
        foreach($meses as $mes) :
            ...
            $data_inicio = (new DateTime())->setDate(date('Y'), $mes['mes'], 1)->format('Y-m-d');
            $data_final  = (new DateTime())->setDate(date('Y'), $mes['mes'], 31)->format('Y-m-d');


            $notas_nfe =  null; $this->nfe_model
                                ->where('id_contador', $this->id_contador)
                                ->where('id_empresa', $this->id_empresa)
